...
class Element(BaseModel):
    ...
    @classmethod
    def get_sub_elements(cls, _id: str, child_class: Element, user):
        privileges = user.privileges.get_privilege(child_class, _id)
        filters = {"parentElementId": _id}
        project = ["_id", "name"]
        if child_class.__name__ == "Product":
            project.append("sub_elements")
        if privileges != "All":
            filters['_id__in'] = privileges

        return list(child_class.objects(**filters).only(*project))
    ...
    def get_elements(cls):
        ...
        return list(cls.objects(_id__in=privileges))

    @classmethod
    def get_element(cls, element_id):
        element = cls.objects(id=element_id)
        if element:
            return element[0]
